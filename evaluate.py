...
def evaluate(
    image_dir: str, model_data: dict, board_metadata: list
) -> Tuple[float, float]:
    ...
    # for i in fvectors_reduced[0]:
    #     print(i)
    # print(len(fvectors_reduced[0]))
    # print(len(fvectors_reduced))
    ...
    score_board = (
        100.0 * np.sum(output_labels_board == np.array(true_labels)) / len(true_labels)
    )
    ...
    score_square = (
        100.0 * np.sum(output_labels_squares == true_labels) / len(true_labels)
    )
